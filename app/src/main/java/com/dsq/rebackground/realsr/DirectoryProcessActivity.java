...
public class DirectoryProcessActivity extends AppCompatActivity {
    ...
    private void loadSettings() {
        ...
        if (commandList.length == 0) {
            ToastUtil.showToast(this, R.string.dir_no_supported_commands, Toast.LENGTH_LONG);
        }
    ...
    private void startBatchProcess() {
        if (!isBound || processingService == null) {
            ToastUtil.showToast(this, R.string.dir_service_error, Toast.LENGTH_SHORT);
            return;
        }
        ...
        if (inputPath.isEmpty()) {
            ToastUtil.showToast(this, R.string.dir_input_path_error, Toast.LENGTH_SHORT);
            return;
        }

        File inputDir = new File(inputPath);
        if (!inputDir.exists() || !inputDir.isDirectory()) {
            ToastUtil.showToast(this, R.string.dir_input_invalid, Toast.LENGTH_SHORT);
            return;
        }

        if (outputPath.isEmpty()) {
            ToastUtil.showToast(this, R.string.dir_output_path_error, Toast.LENGTH_SHORT);
            return;
        }

        int modelIndex = spinnerModel.getSelectedItemPosition();
        if (modelIndex < 0 || modelIndex >= commandList.length) {
            ToastUtil.showToast(this, R.string.dir_model_error, Toast.LENGTH_SHORT);
            return;
        }
        ...
        processingService.startTask(execCmd, dir, notifySetting, new ImageProcessor.ProcessCallback() {
            ...
            public void onCompleted(String result, boolean success) {
                runOnUiThread(() -> {
                    ...
                    if (success) {
                        ToastUtil.showToast(DirectoryProcessActivity.this, R.string.save_succeed, Toast.LENGTH_LONG);
                    }
                });
            }
